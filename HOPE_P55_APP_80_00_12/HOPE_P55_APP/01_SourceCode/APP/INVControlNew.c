...
INT16S	g_wCtrlVoltFlag = 0;		// 整个控制环路输出
...
INT16S g_wInvFeedBusVoltRef = 4000;//4608;//4140;		// 并网BUS电压参考 414V = 23V × 18变比
...
void	sFBBUSController2(void)// 并网BUS电压外环
{
	...
	if(++g_wLoopCnt >= 8)
	{
		...
		if(g_wBusVoltErr > cBusVoltReal30V)			// 20V
		{
			g_wBusVoltErr = cBusVoltReal30V;
		}
		else if(g_wBusVoltErr < -cBusVoltReal30V)
		{
			g_wBusVoltErr = -cBusVoltReal30V;
		}
...
////////////////////////////电池电压外环
//INT16S  g_wLoopCnt = 0;
INT32S  g_dwPBatVoltSum = 0;
INT16S  g_wPBatVoltAvg = 0;
F32 g_wPBatVoltErr = 0;
F32 g_wBatVoltErr = 0;

F32 fBVmoMax = 0;
F32 fBat_P = 0;
F32 fBat_I = 0;
F32 fBatVmo = 0;                // BAT PI控制器输出
F32 fBATVKp = 0.05;             // KP系数
F32 fBATVKi = 0.0005;           // KI系数
///////////////////////////////////电池电压外环母线电压限幅
INT32S  g_dwPBusVoltSum2 = 0;
INT16S  g_wPBusVoltAvg2 = 0;
F32 g_wPBusVoltErr2 = 0;
F32 g_wBusVoltErr2 = 0;

//F32 fBVmoMax = 0;
F32 fBus_P2 = 0;
F32 fBus_I2 = 0;
F32 fBusVmo2 = 0;                // BAT PI控制器输出
F32 fBusVKp2 = 0.05;             // KP系数
F32 fBusVKi2 = 0.0005;           // KI系数
...
{
	g_dwPBusVoltSum += swGetAdcBusVoltReal();
	g_dwPBatVoltSum += swGetAdcBatVoltReal();
	...
	if(++g_wLoopCnt >= 8)
	{
		...
		g_wPBusVoltErr = g_wInvFeedBusVoltRef - g_wPBusVoltAvg;
		g_wBusVoltErr = g_wPBusVoltErr;
		if(g_wBusVoltErr > cBusVoltReal10V)			// 20V
		{
			g_wBusVoltErr = cBusVoltReal10V;
		}
		else if(g_wBusVoltErr < -cBusVoltReal10V)
		{
			g_wBusVoltErr = -cBusVoltReal10V;
		}
		...
		////////////////////////////////////////////////////////////电池电压外环  母线电压限幅
		g_wPBusVoltErr2 = 4850 - g_wPBusVoltAvg;			//控制母线最大电压470V                          暂定
		g_wBusVoltErr2 = g_wPBusVoltErr2;
		if(g_wBusVoltErr2 > cBusVoltReal10V)			// 20V
		{
			g_wBusVoltErr2 = cBusVoltReal10V;
		}
		else if(g_wBusVoltErr2 < -cBusVoltReal10V)
		{
			g_wBusVoltErr2 = -cBusVoltReal10V;
		}
		
		// Bus voltage loop PI controler
		fBus_P2 = g_wBusVoltErr2 * fBusVKp2;									// 计算KP参数
		fBus_I2 += g_wBusVoltErr2 * fBusVKi2;									// 计算KI参数
		UpDownLimit(fBus_I2,gi_wInvChgCurrLimit,-gi_wInvFeedCurrLimit);		// KI限幅
		fBusVmo2 = fBus_P2 + fBus_I2;											// PI环路输出
		UpDownLimit(fBusVmo2,gi_wInvChgCurrLimit,-gi_wInvFeedCurrLimit);		// PI输出限幅(实际控制电流最大值)
		////母线最大电压环输出fBusVmo2

		////////////////////////////////////////////////电池电压外环	
        g_wPBatVoltAvg = g_dwPBatVoltSum >> 3;
        g_dwPBatVoltSum = 0;

        // 误差值
        g_wPBatVoltErr = swGetBatVoltRef() - g_wPBatVoltAvg;
        g_wBatVoltErr = g_wPBatVoltErr;
        if(g_wBatVoltErr > cBatVoltReal1V)         // 20V
        {
            g_wBatVoltErr = cBatVoltReal1V;
        }
        else if(g_wBatVoltErr < -cBatVoltReal1V)
        {
            g_wBatVoltErr = -cBatVoltReal1V;
        }

        // Bat voltage loop PI controler
        fBat_P = g_wBatVoltErr * fBATVKp;                                   // 计算KP参数
        fBat_I += g_wBatVoltErr * fBATVKi;                                  // 计算KI参数
        UpDownLimit(fBus_I,gi_wInvChgCurrLimit,-gi_wInvFeedCurrLimit);      // KI限幅
        fBatVmo = fBat_P + fBat_I;                                          // PI环路输出
        UpDownLimit(fBatVmo,gi_wInvChgCurrLimit,-gi_wInvFeedCurrLimit);     // PI输出限幅(实际控制电流最大值)
		UpDownLimit(fBatVmo,fBusVmo2,-fBusVmo2);   //限制电池电压环路最大母线电压

        //两个电压环取最大值
        if(fBusVmo > fBatVmo)
        {
        	g_wCtrlVoltFlag = 0;
            fBVmoMax = fBusVmo;   //运行母线电压环
        }
        else
        {
        	g_wCtrlVoltFlag = 1;
            fBVmoMax = fBatVmo;//运行电池电压环
        }
//        UpDownLimit(fBVmoMax,fBatVmo,-fBatVmo);
		if(fBVmoMax > fBatVmo)
        {
            fBVmoMax = fBatVmo;
        }
        else if(fBVmoMax < -fBatVmo)
        {
            fBVmoMax = -fBatVmo;
        }
		
		fPDCDCImo = fBVmoMax;												// 外环输出，限制在系统额定最大充放电电流
		UpDownLimit(fPDCDCImo,cMaxInvFeedCntlCurr1,-cMaxInvFeedCntlCurr1);	// 0.1A
//		UpDownLimit(fPDCDCImo,cMaxInvFeedCntlCurr,-cMaxInvFeedCntlCurr);	// 0.01A
	}
...
{
	// 50hz Kp=2.5, Kr=280, Wc=2×3.14×5=31.4, Wo=2×3.14×50=314	Ts=19.2k
//	fKcd1 = 1.9965f;	// 1 旧款机器
//	fKcd2 = 0.9967f;
//	fKcn0 = 2.9571f;
//	fKcn1 = 4.9912f;
//	fKcn2 = 2.0347f;

	// fKcd1 = 1.9965f;	// 2 上周验证有效
	// fKcd2 = 0.9967f;
	// fKcn0 = 3.9796f;
	// fKcn1 = 5.9894f;
	// fKcn2 = 2.0106f;

//	fKcd1 = 1.9965f;	// 3 使用2的参数降级使用，可以正常并网
//	fKcd2 = 0.9967f;
//	fKcn0 = 3.3163f;
//	fKcn1 = 4.9912f;
//	fKcn2 = 1.6755f;

// 50hz Kp=4, Kr=800, Wc=2×3.14×5=31.4, Wo=2×3.14×50=314	Ts=19.2k
	fKcd1 = 1.9965f;	// 2 上周验证有效
	fKcd2 = 0.9967f;
	fKcn0 = 5.3061f;
	fKcn1 = 7.9859f;
	...
	// 150hz Kp=4, Kr=800, Wc=2×3.14×1=6.28, Wo=2×3.14×150=942	Ts=19.2k
//	fKc2d1 = 1.9969f;
//	fKc2d2 = 0.9993f;
//	fKc2n0 = 4.2614f;
//	fKc2n1 = 7.9878f;
...
{
	// 60hz Kp=2.5, Kr=280, Wc=2×3.14×5=31.4, Wo=2×3.14×60=376.8	Ts=19.2k
	fKcd1 = 1.9964f;
	fKcd2 = 0.9967f;
	fKcn0 = 2.9571f;
	fKcn1 = 4.9909f;
	...
	// 180hz Kp=2.5, Kr=280, Wc=2×3.14×1=6.28, Wo=2×3.14×180=1,130.4	Ts=19.2k
	fKc2d1 = 1.9959f;
	fKc2d2 = 0.9993f;
	fKc2n0 = 2.5915f;
	fKc2n1 = 4.9897f;
...
void	sFBINVController2(void)
{
	...
	fDisChgCurrSamp = (F32)(-swGetAdcInvCurrReal()/10);		// 实际采样数据  		// 0.1A 采样改成了0.01A 改成0.01后并网失败，初始电流太大
	...
	UpDownLimit(fDisChgCurrLoopOut0,3850.0,-3850.0);
...
//	fDisChgPwm = fDisChgCurrLoopOut0 + fDisChgCurrLoop2Out0 * g_uwGridVoltFeedWard; //\
	...
	fDisChgPwm = fDisChgCurrLoopOut0 + swGetAdcInvVoltReal() * g_uwGridVoltFeedWard;
	
	UpDownLimit(fDisChgPwm,3850.0,-3850.0);
...
INT16S	swGetCtrlVoltFlag(void)					
{
	return g_wCtrlVoltFlag;
}
