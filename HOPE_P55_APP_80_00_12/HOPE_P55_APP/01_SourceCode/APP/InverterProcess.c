...
void	sInvParaUpdate(void)
{
	...
	else if(g_uwModelVar == cCtrlHOPE2000)
	{
		...
		strInvParameterData.Field.uwCTdirection = cReverse;						// OP电流采样CT方向
		strInvParameterData.Field.uwInvCurrRatio = 6876;						// 逆变电流采样系数
		strInvParameterData.Field.uwInvWattRes = 5290;							// = (2200 * 2200 / 1000)
		...
		// 电池参数
		strInvParameterData.Field.uwSettingMaxChgCurr = 2000;					// 电池最大充电电流 160A	// 精度可能有问题
		strInvParameterData.Field.uwSettingMaxDcgCurr = 2600;					// 电池最大放电电流 180A	// 精度可能有问题
		strInvParameterData.Field.uwSettingMaxACChgCurr = 2000;				// 市电最大充电电流 160A	// 精度可能有问题
		...
		strInvParameterData.Field.uwInvRateVolt = cAcVoltReal230V;				// 逆变额定电压 230V		
		strInvParameterData.Field.uwInvDeratingVolt = cAcVoltReal150V;			// 逆变降额最低电压
		strInvParameterData.Field.uwInvFreeCntlCurrMax = 85;//71;    				// 3.535A(1000W*1.414/200V=7.07A)
		strInvParameterData.Field.uwInvVoltOver = cAcVoltReal280V;    			// 逆变输出过压值 280V
		strInvParameterData.Field.uwInvVoltOverBack = cAcVoltReal270V;    		// 逆变输出过压恢复值 270V
		...
		strInvParameterData.Field.uwEnergySpareVolt = 1900;						// 绿电模式限制逆变电压190V；
...
void	sInvPowerConversion(void)	// 功率折算
{
	...
	if(swGetWorkMode() == cLineMode || swGetWorkMode() == cChgMode )	// for test 
	{
		// 没开馈电使能时，不显示负功率
...
INT16S	uwGetEnergySpareVolt(void)
{
	return strInvParameterData.Field.uwEnergySpareVolt;
}
