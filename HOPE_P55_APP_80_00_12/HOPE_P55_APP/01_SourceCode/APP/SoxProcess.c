...
{
	wEnergyPct80Volt = 3450;//350;//swGetEepromCellVoltPct80();
	wEnergyPct20Volt = 3050;//250;//swGetEepromCellVoltPct20();
	wEnergyPct20TwoVolt = 3050;//230;//swGetEepromCellVoltPct20Two();
	wEnergyPct05Volt = wEnergyPct20Volt - 100;
	wEnergyCompCoeff = 20;//0;//swGetEepromCellTempCoef();
	
	wEnergyAhRated = suwGetEEBatCapacity();
	...
	wEnergyAhReal = suwGetEEBattAhReal();
	wEnergyAhComp = wEnergyAhReal;
	if(suwGetEEBattAhMax() >= wEnergyAhRated)
	{
		wEnergyAhMax = wEnergyAhRated;
	}
	else
	{
		wEnergyAhMax = suwGetEEBattAhMax();
	}
	...
	sSocShow.wEnergyAhRealShow = suwGetEEBattAhRealShow();
...
void	sSoxOcvChk(INT16U wFilter1, INT16U wFilter2)
{
	...
	{
		fSoxStatus.bits.SocEmpty = false;////sbGetCellVoltLoAlm();	// 2.5V
		fSoxStatus.bits.SocFull = false;//(T_FAULT_DATA.bits.bBatSoftOV);//(T_ALARM_DATA.bits.bBatOVAlarm);//false;//(sbGetCellVoltHiAlm() || sbGetCellVoltFullFlag());		// 3.6V
...
//		}
		...
		//if(fSoxStatus.bits.SocOcvPause == false)	// for test  20269325
		if(0)
		{
			if(fSoxStatus.bits.SocLess05Pct == true)
			{
				//if(sb8UOverLevelChk(swGetMinCellVoltFiltNew(),wEnergyPct05Volt,wFilter2,&bSocPct05Cnt) == true)	// BMS
				if(sb8UOverLevelChk(swGetMinCellVoltFilter(),wEnergyPct05Volt,wFilter2,&bSocPct05Cnt) == true)		// HOPE
				{
					fSoxStatus.bits.SocLess05Pct = false;
				}
			}
			...
			{
				//if(sb8UUnderLevelChk(swGetMinCellVoltFiltNew(),wEnergyPct05Volt,wFilter1,&bSocPct05Cnt) == true)	// BMS
				if(sb8UUnderLevelChk(swGetMinCellVoltFilter(),wEnergyPct05Volt,wFilter1,&bSocPct05Cnt) == true)	// HOPE
				{
			...
			{
				if(sb8UOverLevelChk(swGetMinCellVoltFilter(),wEnergyPct20Volt,wFilter2,&bSocPct20Cnt) == true)
				{
					fSoxStatus.bits.SocLess20Pct = false;
				}
			}
			...
			{
				if(sb8UUnderLevelChk(swGetMinCellVoltFilter(),wEnergyPct20Volt,wFilter1,&bSocPct20Cnt) == true)
				{
			...
			{
				if(sb8UUnderLevelChk(swGetMaxCellVoltFilter(),wEnergyPct80Volt,wFilter2,&bSocPct80Cnt) == true)
				{
					fSoxStatus.bits.SocLarge80Pct = false;
				}
			}
			...
			{
				if(sb8UOverLevelChk(swGetMaxCellVoltFilter(),wEnergyPct80Volt,wFilter1,&bSocPct80Cnt) == true)
				{
			...
			if(swGetSocNew() < 20)
			{
				...
				{
					if(sb8UUnderLevelChk(swGetMinCellVoltFilter(), wEnergyPct20TwoVolt, wFilter2,&bSocPct20Cnt2) == true)
					{
						fSoxStatus.bits.SocLess20Pct2 = false;
					}
				}
				else
				{
					...
					if(sb8UOverLevelChk(swGetMinCellVoltFilter(), wTemp, wFilter1,&bSocPct20Cnt2) == true)
					{
...
void	sSoxAhCalc(INT16S wCurr)
{
	...
	//if((bBmsMode == cBatteryMode) || (bBmsMode == cChargeMode) \
		|| (bBmsMode == cDischargeMode))	//BMS
//	if(hoSRDRV)	//  充放电MOS管使能				// HOPE 20260312
	...
	if(dwAhAcc <= -36000)	// 36000 = 60min = 60min × 60sec  × 10(10 = 任务100ms)
	{
		dwAhAcc += 36000;
	...
	else if(dwAhAcc >= 36000)
	{
		...
		else if(swGetMinCellTempNew()/10 >= 20)	// 20℃
		{
	...
	else
	{
		...
		if((swGetMinCellTempNew()/10) <= 15)	// 15℃
		{
			fSoxStatus.bits.SohAccEn = false;
		}
	}
...
void	sSoxAhCalc_Show(INT16S wCurr)
{
	...
	//if((bBmsMode == cBatteryMode) || (bBmsMode == cChargeMode) || (bBmsMode == cDischargeMode))	// BMS
	//if(hoSRDRV)	//  充放电MOS管使能	// HOPE 20260312
	if(1)
	{
		dwAhAccShow += wCurr;//(wCurr * sSocShow.wCurrIntegralShow) / 10;
//		dwAhAccShow += (wCurr * (INT16S)sSocShow.wCurrIntegralShow) / 10;
	}
	else
	{
		if(++bSelfAccCnt >= 100)
	...
	if(++wSocCnt >= wFilter)
	{
		...
		else
		{
			...
			if(wCurrIntegralEN == 1)
			{
				...
				//wBmsStatusLow = swGetBmsDataBmsStatusLow(sbGetSciAddress());	// BMS
				if(sdiff > 0)
				{ 
					//if(wBmsStatusLow & 0x2000)		// BMS
					if(swGetBatCurrNew() > 0)	 // 充电	 // HOPE
					{
						sSocShow.wCurrIntegralShow = scale;           // 充电 → 加速
					}
					//else if(wBmsStatusLow & 0x1000)// BMS
					else if(swGetBatCurrNew() < 0)	 // 放电	 // HOPE
					{
