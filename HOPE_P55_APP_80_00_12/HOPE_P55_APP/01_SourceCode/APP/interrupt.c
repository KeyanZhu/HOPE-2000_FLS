...
INT16U gi_uwBusVoltHiIntCnt = 0;	// BUS软件过压计数
INT16U gi_uwBusVoltHWHiIntCnt = 0;	// BUS硬件过压计数
INT16U gi_uwBatVoltHiIntCnt = 0;	// 电池软件过压计数
INT16U gi_uwBatVoltHWHiIntCnt = 0;	// 电池硬件过压计数
INT16U gi_uwInvCurrHWHiIntCnt = 0;
INT16U gi_uwPVCurrHWHiIntCnt = 0;
...
INT16U gi_uwPWM1_TZ_Cnt = 0;
INT16U gi_uwPWM2_TZ_Cnt = 0;
INT16U gi_uwPWM3_TZ_Cnt = 0;
INT16U gi_uwPWM1_TZ_ResetCnt = 0;
INT16U gi_uwPWM2_TZ_ResetCnt = 0;
INT16U gi_uwPWM3_TZ_ResetCnt = 0;
...
interrupt void isrINV(void)
{
	...
	AdcaRegs.ADCSOCFRC1.all = 0X7FFF;// Software triggered ADC conversion
	AdcbRegs.ADCSOCFRC1.all = 0X03FF;// Software triggered ADC conversion
	AdccRegs.ADCSOCFRC1.all = 0X0001;// Software triggered ADC conversion
	...
	if(EPwm2Regs.TZFLG.bit.DCAEVT2)
	{
		EALLOW; 
		...
		EDIS;
		gi_uwPWM2_TZ_ResetCnt = 0;
		if(++gi_uwPWM2_TZ_Cnt > 50)
		{
			gi_uwPWM2_TZ_Cnt = 0;
			mDisINVPWMOut();
			sSetFBInvCtrlSts(cFBWait);
	...
	else if(gi_uwPWM2_TZ_Cnt > 0)
	{
		if(++gi_uwPWM2_TZ_ResetCnt > 10)
		{
			gi_uwPWM2_TZ_ResetCnt = 0;
			gi_uwPWM2_TZ_Cnt--;
		}
	...
	wInvVoltTemp = swGetAdcInvVoltReal();
	if(wInvCurrTemp >= cInv14A || wInvCurrTemp < -cInv14A)	// 继电器30A,1.5倍就是45A
	{
		g_wRInvOverCurrCnt++;
		g_wRInvOverCurrPCnt++;
		wRInvOverCurrFlg = 1;
	...
	if(wInvVoltTemp >= cAcVoltReal400V || wInvVoltTemp < -cAcVoltReal400V)	
	{
		sSetInvPWMEn(false);
	...
	if(swGetAdcBusVoltReal() >= 5000)//uwGetBusVoltOver3())	// BUS电容是550V，触发后会封逆变
	{
		wPBusOverVoltFlg = 1;
	}
	else
...
interrupt void isrDCDC(void)
{
	...
	if(!mBUSVOVER)
	{
		if(++gi_uwBusVoltHWHiIntCnt >= 1)
		{
			gi_uwBusVoltHWHiIntCnt = 1;
			sSetPV1CtrlSts(cPVWait);
			T_FAULT_DATA.bits.bBusHardOV = true;
		}
	}
	else if(gi_uwBusVoltHWHiIntCnt > 0)
	{
		gi_uwBusVoltHWHiIntCnt--;
	}

	// BAT硬件过流检测
	if(!mBUSVOVER)
	{
		if(++gi_uwBatVoltHWHiIntCnt >= 1)
		{
			gi_uwBatVoltHWHiIntCnt = 1;
			sSetPV1CtrlSts(cPVWait);
			T_FAULT_DATA.bits.bBusHardOV = true;
		}
	}
	else if(gi_uwBatVoltHWHiIntCnt > 0)
	{
		gi_uwBatVoltHWHiIntCnt--;
	}
	
	// PV硬件过流检测
	if(!mPVIOVER)
	{
		if(++gi_uwPVCurrHWHiIntCnt >= 1)
		{
			gi_uwPVCurrHWHiIntCnt = 1;
			sSetPV1CtrlSts(cPVWait);
			T_ALARM_DATA.bits.bPv1HWOCAlarm = true;
		}
	}
	else if(gi_uwPVCurrHWHiIntCnt > 0)
	{
		gi_uwPVCurrHWHiIntCnt--;
	}

	// BUS快速过压检测
	if(swGetAdcBusVoltReal() >= 5000)	// BUS电容是500V
	{
	...
	// 电池快速过压检测
	if(swGetBatVoltNew() >= (cBatVoltReal3V75 * uwGetBatSerialPcs()) && (swGetWorkMode() != cPowerOnMode))
	{
	...
	else
	{
		...
		{
//			mPV_FANENOn;	//打开PV防反信号
			sPV1Control();		// HOPE系列 PV闭环控制(无软起，直接就是work模式)
		}
...
void	sSetRSinPionterQ15(INT16U uwState)
{
	g_dwRSinPionterQ15 = uwState;
}
