...
void	sInverterPhaseLock(INT16U wInvPeriodCnt,INV_TD *pInvTd,INT16U wRefPeriodCnt,INT16U wStep)
{
	...
	if((g_uwPhaseDelta <= g_uwPhaseOKLimit) && (g_uwPeriodDelta <= g_uwPeriodOKLimit))
	{						// g_uwPhaseOKLimit = 250				g_uwPeriodOKLimit = 200
	...
	{
		s_wPhaseLockCnt = 0;
		if(g_uwPhaseDelta > g_uwPhaseLossLimit)	// g_uwPhaseLossLimit = 2000
		{
			OS_ENTER_CRITICAL();
...
void	sFrePhaseLockProc(void)
{
	...
	&& swGetWorkMode() != cPowerOnMode && (swGetRLineVoltFilter() >= 850))
	{
...
void	sRVoltRegu(INT16U wRTrackVolt, INT16U wRKVoltrack1, INT16U wRKVoltrack2)
{
	...
	if(wRVoltError >= 600)	// 100
	{
		wRVoltError = 600;
	}
	else if(wRVoltError <= -600)
	{
		wRVoltError = -600;
	}
...
void	sInvVoltHighChk(INT8U ubFilter,INT8U ubFilter1)
{
	...
	{
		if((ubInvVoltHigh && (sb16UUnderLevelChk(wRInvVoltNew, uwGetInvVoltOverBack(), ubFilter, &s_uwRInvVoltHighCnt)))\
		||(ubInvVoltLow && (sb16UOverLevelChk(wRInvVoltNew, 0, ubFilter1, &s_uwRInvVoltLowCnt))))
		{
			if(ubInvVoltHigh == true) ubInvVoltHigh = false;
			if(ubInvVoltLow == true) ubInvVoltLow = false;
	...
	{
		if(swGetWorkMode() == cBatteryMode && (suwGetFBInvCtrlSts() == cFBInvCtrlVolt))
//		if((suwGetFBInvCtrlSts() == cFBInvCtrlVolt || suwGetFBInvCtrlSts() == cFBInvOpenLoop))
		{
			...
			if(sb16UUnderLevelChk(wRInvVoltNew, cAcVoltReal90V, ubFilter1, &s_uwRInvVoltLowCnt))
			{
				ubInvVoltLow = true;
			}
			
			if(ubInvVoltHigh || ubInvVoltLow)
			{
				s_uwRInvVoltHighCnt = 0;
				s_uwRInvVoltLowCnt = 0;
