...
//		g_uwTurboMode = true;
	...
	wWattTemp = swGetROpWattNew();//uwGetInvWattMax()
	if((wWattTemp > 1000) && (swGetWorkMode() == cBatteryMode)&&(g_uwTurboMode == true))
	{
		uwWattDeratePerTemp = (wWattTemp - uwGetInvWattMax()) / 2;
...
//	if(mBATONS == false)
//	{
//		if((++s_uwShutdownCount > cT100ms) && (uwShutdownSts == false))
//		{
//			uwShutdownSts = true;
...
//			//if(++s_uwToBatModeCount > cT20sec)
//			//if((++s_uwToBatModeCount > cT20sec) && (swGetBusVoltNew() <= cBusVoltReal60V))	// 并且BUS电压低于60V
...
INT16S sOPOverLoadLowvolt(INT16S wOpVoltTemp)
{
	...
	if(wOPLOadTemp > 1050)
	{
		...
		if(((wOPLOadTemp >= 2000) && (g_uwOPLoadLevelCnt > 0))\
		|| ((wOPLOadTemp <  2000) &&(wOPLOadTemp >= 1700) && (g_uwOPLoadLevelCnt > cT500ms))\
		|| ((wOPLOadTemp <  1700) &&(wOPLOadTemp >= 1400) && (g_uwOPLoadLevelCnt > cT1sec5))\
		|| ((wOPLOadTemp <  1400) &&(wOPLOadTemp >= 1200) && (g_uwOPLoadLevelCnt > cT25sec))\
		|| ((wOPLOadTemp <  1200) &&(wOPLOadTemp >= 1050) && (g_uwOPLoadLevelCnt > cT50sec)))
		{
			g_uwTurboMode = true;
		...
		else if(((wOPLOadTemp >= 2000) && (g_uwOPLoadLevelCnt > cT50ms))\
			|| ((wOPLOadTemp <  2000) &&(wOPLOadTemp >= 1700) && (g_uwOPLoadLevelCnt > cT250ms))\
			|| ((wOPLOadTemp <  1700) &&(wOPLOadTemp >= 1400) && (g_uwOPLoadLevelCnt > cT2sec))\
			|| ((wOPLOadTemp <  1400) &&(wOPLOadTemp >= 1200) && (g_uwOPLoadLevelCnt > cT30sec))\
			|| ((wOPLOadTemp <  1200) &&(wOPLOadTemp >= 1050) && (g_uwOPLoadLevelCnt > cT1min)))
		{
			g_uwTurboMode = false;
	...
	*/
	...
	else if(wOPLOadTemp > 1700)
	{
		g_uwOPLoadLevelCnt++;
		
		if(g_uwOPLoadLevelCnt > cT500ms) 
		{
			g_uwTurboMode = false;
			T_FAULT_DATA.bits.bLoadOverLoad = true;
			sSetFaultCode(cOverLoad);
		...
		else if(g_uwOPLoadLevelCnt > cT250ms)
		{
			g_uwTurboMode = true;
			if(wVolt > wWattDeratROpVolt) 		
			{
	...
	else if(wOPLOadTemp > 1200)
	{
		g_uwOPLoadLevelCnt++;
		
		if(g_uwOPLoadLevelCnt > cT8sec) 
		{
			g_uwTurboMode = false;
			T_FAULT_DATA.bits.bLoadOverLoad = true;
			sSetFaultCode(cOverLoad);
		...
		else if(g_uwOPLoadLevelCnt > cT3sec)
		{
			g_uwTurboMode = true;
			if(wVolt > wWattDeratROpVolt) 		
			{
	...
	else if(wOPLOadTemp > 1050)	
	{
		g_uwOPLoadLevelCnt++;
		
		if(g_uwOPLoadLevelCnt > cT30sec) 
		{
			g_uwTurboMode = false;
			T_FAULT_DATA.bits.bLoadOverLoad = true;
			sSetFaultCode(cOverLoad);
		...
		else if(g_uwOPLoadLevelCnt > cT25sec)
		{
			g_uwTurboMode = true;
			if(wVolt > wWattDeratROpVolt) 		
			{
	...
	else
	{
		...
		{
			if(g_uwOPLoadLevelCnt > 60) g_uwOPLoadLevelCnt = 60;
		}
		if(g_uwOPLoadLevelCnt > 0) g_uwOPLoadLevelCnt--;
		wVolt++;
	}
