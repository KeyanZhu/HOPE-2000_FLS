...
INT16U	uwGridpowerslopeTime = 180;//240;//60;	// 电网功率软起斜率
...
//	}
//	if(swGetOPPercent() > 200)

	sOPOverLoadLowvolt();
/*
	if(swGetROpWattNew() > 2000)		
	{
		g_uwOPLoadLevelCnt++;
		if(g_uwOPLoadLevelCnt > 0)
		{
			if(wOutputVoltCntl > wWattDeratROpVolt) 		
			{
				wOutputVoltCntl = wWattDeratROpVolt;
		...
		else if(g_uwOPLoadLevelCnt > cT50ms) 
		{
	...
	//	else if(swGetOPPercent() > 170)
	else if(swGetROpWattNew() > 1700)
	{
		g_uwOPLoadLevelCnt++;
		if(g_uwOPLoadLevelCnt > cT250ms)
		{
			if(wOutputVoltCntl > wWattDeratROpVolt) 		
			{
				wOutputVoltCntl = wWattDeratROpVolt;
		...
		else if(g_uwOPLoadLevelCnt > cT500ms) 
		{
			T_FAULT_DATA.bits.bLoadOverLoad = true;
			sSetFaultCode(cOverLoad);
			g_uwOPLoadLevelCnt = 0;
...
//	else if(swGetOPPercent() > 140)
	else if(swGetROpWattNew() > 1400)
	{
		g_uwOPLoadLevelCnt++;
		if(g_uwOPLoadLevelCnt > cT1sec5)
		{
			if(wOutputVoltCntl > wWattDeratROpVolt) 		
			{
				wOutputVoltCntl = wWattDeratROpVolt;
		...
		else if(g_uwOPLoadLevelCnt > cT2sec) 
		{
			T_FAULT_DATA.bits.bLoadOverLoad = true;
			sSetFaultCode(cOverLoad);
			g_uwOPLoadLevelCnt = 0;
...
//	else if(swGetOPPercent() > 120)
	else if(swGetROpWattNew() > 1200)
	{
		g_uwOPLoadLevelCnt++;
		if(g_uwOPLoadLevelCnt > cT25sec)
		{
			if(wOutputVoltCntl > wWattDeratROpVolt) 		
			{
				wOutputVoltCntl = wWattDeratROpVolt;
		...
		else if(g_uwOPLoadLevelCnt > cT30sec) 
		{
			T_FAULT_DATA.bits.bLoadOverLoad = true;
			sSetFaultCode(cOverLoad);
			g_uwOPLoadLevelCnt = 0;
...
//	else if(swGetOPPercent() > 105)
	else if(swGetROpWattNew() > 1050)	
	{
		g_uwOPLoadLevelCnt++;
		if(g_uwOPLoadLevelCnt > cT50sec)
		{
			if(wOutputVoltCntl > wWattDeratROpVolt) 		
			{
				wOutputVoltCntl = wWattDeratROpVolt;
		...
		else if(g_uwOPLoadLevelCnt > cT1min) 
		{
			T_FAULT_DATA.bits.bLoadOverLoad = true;
			sSetFaultCode(cOverLoad);
			g_uwOPLoadLevelCnt = 0;
...
void	sPvLowVoltChk(void)	//名字不合适，后续改
{
	...
	{
		mPVLOADENOn;	// PV弱光吸收电阻，默认关闭
	}
	else								// == false,表示PV存在或者PV强，吸收电路关闭
	{
		mPVLOADENOff;	// PV弱光吸收电阻，默认关闭
	}
}
...
INT16U	uwBatChgVoltChk(void)	// 待定生效，暂时不生效(一体机不用)
{
	...
	{
		uwBatChgVolt = cBatVoltReal3V65 * cBatSerialPcs;
	}
...
//	UpDownLimit(wInvCtrlPower,swGetTraTempDeratPowerLimit(),-swGetTraTempDeratPowerLimit());	// 推挽变压器温度降额功率
	...
	UpDownLimit(wInvCtrlPower,swGetLineVoltDeratPowerLimit(),-swGetLineVoltDeratPowerLimit());	// 市电电压降额
