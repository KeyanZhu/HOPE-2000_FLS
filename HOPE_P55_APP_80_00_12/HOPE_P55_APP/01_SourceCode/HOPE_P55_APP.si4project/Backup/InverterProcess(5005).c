...
//		strInvParameterData.Field.uwHsTwTempEndDerat = cHsTemp90C;				// 逆变散热器停止降额温度
	...
	else if(g_uwModelVar == cCtrlHOPE2000)
	{
		...
		// 逆变参数
		strInvParameterData.Field.uwInvVAMax = 1000;							// 额定视在功率     //充电达不到满功率
		strInvParameterData.Field.uwInvWattMax = 1000;							// 额定有功功率
		strInvParameterData.Field.uwInvRateVolt = cAcVoltReal230V;				// 逆变额定电压 230V
		strInvParameterData.Field.uwInvDeratingVolt = cAcVoltReal150V;			// 逆变降额最低电压
		strInvParameterData.Field.uwInvFreeCntlCurrMax = 71;	    			//待调试    // 7.07A(1000W*1.414/200V=7.07A)	// 0.1A
		strInvParameterData.Field.uwInvVoltOver = cAcVoltReal280V;    			// 逆变输出过压值 280V
		strInvParameterData.Field.uwInvVoltOverBack = cAcVoltReal270V;    		// 逆变输出过压恢复值 270V
		strInvParameterData.Field.uwInvCurrOver = cInv10A;    					// 逆变输出过流值 10A
//		strInvParameterData.Field.uwHsInvTempStartDerat = cHsTemp85C;			// 逆变散热器开始降额温度
//		strInvParameterData.Field.uwHsInvTempEndDerat = cHsTemp89C;				// 逆变散热器停止降额温度
//		strInvParameterData.Field.uwHsTwTempStartDerat = cHsTemp80C;			// 推挽散热器开始降额温度
//		strInvParameterData.Field.uwHsTwTempEndDerat = cHsTemp90C;				// 推挽器停止降额温度
		strInvParameterData.Field.uwTraTempStartDerat = cHsTemp80C; 			// 推挽变压器温度开始降额温度
		strInvParameterData.Field.uwTraTempEndDerat = cHsTemp85C;				// 推挽变压器温度停止降额温度
...
void	sInverterRealtimeDataUpdate(void)
{
	...
	strInverterRealtimeData.Field.uwBatSocConvertion = 0;
	...
	strInverterRealtimeData.Field.uwPv1InputCurr = 0;
	strInverterRealtimeData.Field.uwPv1InputPower = 0;
...
//	}
	...
	strInverterRealtimeData.Field.wINV_DCI = swGetRInvDcCurrFilter();
	...
	strInverterRealtimeData.Field.uwReserve143 = 0;
	strInverterRealtimeData.Field.uwReserve144 = 0;
	strInverterRealtimeData.Field.uwReserve145 = 0;
	strInverterRealtimeData.Field.uwReserve146 = 0;
	...
	strInverterRealtimeData.Field.wInvTemp = 0;
	...
	strInverterRealtimeData.Field.wBatHTemp = swGetHsBatHTempFilter();
	strInverterRealtimeData.Field.wBatLTemp = swGetHsBatLTempFilter();
	strInverterRealtimeData.Field.wLLCHTemp = swGetHsLLCHTempFilter();
	strInverterRealtimeData.Field.wLLCTraTemp = swGetHsTraTempFilter();
	...
	//Power generation
	strInverterRealtimeData.Field.uwPV1TotalGenEnergyH 	 = (INT16U)(swGetPV1DayEnergy() >> 16);
	strInverterRealtimeData.Field.uwPV1TotalGenEnergyL 	 = (INT16U)(swGetPV1DayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwPV2TotalGenEnergyH 	 = (INT16U)(swGetPV2DayEnergy() >> 16);
	strInverterRealtimeData.Field.uwPV2TotalGenEnergyL 	 = (INT16U)(swGetPV2DayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwBatTotalChargeH 	 = (INT16U)(swGetBatChargDayEnergy() >> 16);
	strInverterRealtimeData.Field.uwBatTotalChargeL 	 = (INT16U)(swGetBatChargDayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwBatTotalDisChargeH   = (INT16U)(swGetBatDisChargDayEnergy() >> 16);
	strInverterRealtimeData.Field.uwBatTotalDisChargeL 	 = (INT16U)(swGetBatDisChargDayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwGridTotalCostEnergyH = (INT16U)(swGetGridDayEnergy() >> 16);
	strInverterRealtimeData.Field.uwGridTotalCostEnergyL = (INT16U)(swGetGridDayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwGridTotalGenEnergyH  = (INT16U)(swGetSupplyGridDayEnergy() >> 16);
	strInverterRealtimeData.Field.uwGridTotalGenEnergyL  = (INT16U)(swGetSupplyGridDayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwLoadTotalCostEnergyH = (INT16U)(swGetLoadDayEnergy() >> 16);
	strInverterRealtimeData.Field.uwLoadTotalCostEnergyL = (INT16U)(swGetLoadDayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwGenTotalCostEnergyH  = (INT16U)(swGetGenDayEnergy() >> 16);
	strInverterRealtimeData.Field.uwGenTotalCostEnergyL  = (INT16U)(swGetGenDayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwSmartLoadTotalDraEnergyH  = (INT16U)(swGetSmartLoadDayEnergy() >> 16);
	strInverterRealtimeData.Field.uwSmartLoadTotalDraEnergyL  = (INT16U)(swGetSmartLoadDayEnergy() & 0x0000FFFF);
	strInverterRealtimeData.Field.uwSmartLoadTotalCostEnergyH = (INT16U)(swGetSmartLoadDayEnergy() >> 16);
	strInverterRealtimeData.Field.uwSmartLoadTotalCostEnergyL = (INT16U)(swGetSmartLoadDayEnergy() & 0x0000FFFF);
	//Res 213~224
...
//
	// BMS	
	strInverterInformationData.Field.uwInflogCurrBmsAddr 	= sbGetBMSAddr();
	strInverterInformationData.Field.uwInflogBmsType 		= swGetBmsInfoType();
	strInverterInformationData.Field.uwInflogBmsSubType 	= swGetBmsInfoSubType();
	strInverterInformationData.Field.uwInflogBmsCommProVer  = swGetBmsInfoCommProVer();
	strInverterInformationData.Field.uwInflogBmsCommInfo 	= swGetBmsInfoCommInfo();
	strInverterInformationData.Field.uwInflogBmsSN1		  	= swGetBmsInfoSerial1();
	strInverterInformationData.Field.uwInflogBmsSN2  		= swGetBmsInfoSerial2();
	strInverterInformationData.Field.uwInflogBmsSN3		  	= swGetBmsInfoSerial3();
	strInverterInformationData.Field.uwInflogBmsSN4		  	= swGetBmsInfoSerial4();
	strInverterInformationData.Field.uwInflogBmsSN5		  	= swGetBmsInfoSerial5();
	strInverterInformationData.Field.uwInflogBmsSNLen		= swGetBmsInfoSNLen();
	strInverterInformationData.Field.uwInflogBmsDispSwVer   = swGetBmsInfoDispSwVer();
	strInverterInformationData.Field.uwInflogBmsMCU1SwVer   = swGetBmsInfoMCU1SwVer();
	strInverterInformationData.Field.uwInflogBmsMCU2SwVer   = swGetBmsInfoMCU2SwVer();
	strInverterInformationData.Field.uwInflogBmsDispHwVer   = swGetBmsInfoDispHwVer();
	strInverterInformationData.Field.uwInflogBmsCtrlHwVer   = swGetBmsInfoCtrlHwVer();
	strInverterInformationData.Field.uwInflogBmsPowerHwVer  = swGetBmsInfoPowerHwVer();
...
{
	return strInvParameterData.Field.uwLLCHTempStartDerat;
}

INT16U	uwGetLLCHTempEndDerat(void)
{
	return strInvParameterData.Field.uwLLCHTempEndDerat;
}
