...
const struct EepromStruct2 strEepromDefaultTable2 = 	
{
	...
	0,			//	uwEEBatSeriesNum;			//	129	电池节数
	0,			//	uwEEBatActivate;			//	130	电池激活
	200,		//	uwEEBatCapacity; 			//	131	电池容量
	0,			//	uwEELithiumMode; 			//	132	锂电池模式
	0,			//	uwEERsvd38; 				//	133 预留
	6000,		//	uwEEBatMaxChgCurr;			//	134	电池最大充电电流限幅值
	16800,		//	uwEEBatMaxDcgCurr;			//	135	电池最大放电电流限幅值
	cBatVoltReal12V*cBatSerialPcs, 			//	uwEEBatVoltage;			//	136	电池额定电压(电池节数)
	0,			//	uwEERsvd42; 				//	137 预留
	1,			//	uwEEGridChargeEn;			//	138	市电充电使能
	6000,		//	uwEEGridMaxChargeCurr;		//	139	市电最大充电电流
	cBatVoltReal11V5*cBatSerialPcs, 		//	uwEEGridStartChargeVolt;	//	140	市电开始充电电池电压
	cBatVoltReal13V5*cBatSerialPcs, 		//	uwEEGridEndChargeVolt;		//	141	市电停止充电电池电压
	...
	0,			//	uwEERsvd56; 				//	151 预留
	cBatVoltReal14V4*cBatSerialPcs, 		//	uwSetBatConstVolt;			//	152	电池恒压充电压
	cBatVoltReal13V6*cBatSerialPcs, 		//	uwSetBatFloatVolt;			//	153	电池浮充电压
	0,			//	uwSetBatFloatTime;			//	154	电池恒充时间
	5,			//	uwEEShutdownSOC;			//	155	电池停止输出SOC
	cBatVoltReal10V5*cBatSerialPcs, 		//	uwEEShutdownVolt;			//	156	电池停止输出电压
	15, 		//	uwEEBatRestartOutSOC; 		//	157	电池重启输出SOC恢复输出SOC
	cBatVoltReal11V5*cBatSerialPcs, 		//	uwEEBatRestartOutVolt;		//	158	电池重启输出电压恢复输出电压
	10, 		//	uwEELowBattSOC;				//	159	电池电量低电量SOC
	cBatVoltReal11V*cBatSerialPcs,			//	uwEELowBattVolt; 			//	160	电池电量低电量电压
	0,			//	uwEERsvd66; 				//	161 预留
	0,			//	uwEERsvd67; 				//	162 预留
	0,			//	uwEEBattResistance; 		//	163 电池线路阻抗
	cBatVoltReal15V*cBatSerialPcs,			//	uwSetBatVoltOverShut; 		//	164	电池过压保护点
	...
	cBatVoltReal14V6*cBatSerialPcs, 		//	uwEEBatEqVolt;				//	172	EQ电压
	...
	0,			//	uwEERsvd98; 				//	193 预留
	0,			//	uwEERsvd99; 				//	194 预留
	0,			//	uwEERsvd100; 				//	195 预留
	0,			//	uwEERsvd101; 				//	196 预留
	0,			//	uwEERsvd102; 				//	197 预留
	0,			//	uwEERsvd103; 				//	198 预留
	...
	//PV
	600, 		//	uwEEPV1StartVolt;			//	224	PV1启动电压
	0, 			//	uwEEPV2StartVolt;			//	225	PV2启动电压
	2000,		//	uwEEPV1CurrrLimit;			//	226	PV1最大输入电流
	0, 			//	uwEEPV2CurrrLimit;			//	227	PV2最大输入电流
	6400,		//	uwEEPV1PowerLimit;			//	228	PV1最大输入功率
	...
	4200,		//	uwEEACOutputRatedPower;		//	241	机器额定功率
	2300,		//	uwEEACOutputRatedVolt;		//	242	额定输出电压
	...
	2100,		//	uwEEPVPowerRlyOn; 			//	256 开启功率（PV）
	80,			//	uwEERlySocOn; 				//	257 锂电池模式下用SOC
	20,			//	uwEERlySocOff; 				//	258 锂电池模式下用SOC
	cBatVoltReal13V6*cBatSerialPcs,			//	uwEERlyVoltOn; 				//	259 自定义模式下用电压
	cBatVoltReal11V5*cBatSerialPcs,			//	uwEERlyVoltOff; 			//	260 自定义模式下用电压
	...
	1,			//	uwPvAutoInvEn;				//	282	单PV无电池开机自动逆变使能,如果有电池此项不生效
...
//	INT16U wTemp;
	...
	if(suwGetEERsvd97() != 0)
	{
		sSetEERsvd97(strEepromDefaultTable2.strBatData.uwEERsvd97);
		bEEPROM2Flag = true;
	}

	if(suwGetEERsvd98() != 0)
	{
		sSetEERsvd98(strEepromDefaultTable2.strBatData.uwEERsvd98);
		bEEPROM2Flag = true;
	}

	if(suwGetEERsvd99() != 0)
	{
		sSetEERsvd99(strEepromDefaultTable2.strBatData.uwEERsvd99);
		bEEPROM2Flag = true;
	}

	if(suwGetEERsvd100() != 0)
	{
		sSetEERsvd100(strEepromDefaultTable2.strBatData.uwEERsvd100);
		bEEPROM2Flag = true;
	}

	if(suwGetEERsvd101() != 0)
	{
		sSetEERsvd101(strEepromDefaultTable2.strBatData.uwEERsvd101);
		bEEPROM2Flag = true;
	}

	if(suwGetEERsvd102() != 0)
	{
		sSetEERsvd102(strEepromDefaultTable2.strBatData.uwEERsvd102);
		bEEPROM2Flag = true;
	}

	if(suwGetEERsvd103() != 0)
	{
		sSetEERsvd103(strEepromDefaultTable2.strBatData.uwEERsvd103);
		bEEPROM2Flag = true;
	}
...
//	if(suwGetEEBatVoltage() != (cBatVoltReal12V*uwBatSerialPcs))
	if((suwGetEEBatVoltage() != cBatVoltReal12V) && (suwGetEEBatVoltage() != cBatVoltReal24V))
	{
		sSetEEBatVoltage(cBatVoltReal12V * uwBatSerialPcs);
		bEEPROM2Flag = true;
	}
	...
	if((suwGetEERlyVoltOn() > cBatVoltReal15V * uwBatSerialPcs) \
		|| (suwGetEERlyVoltOn() < cBatVoltReal11V * uwBatSerialPcs))
	{
		sSetEERlyVoltOn(strEepromDefaultTable2.strAcData.uwEERlyVoltOn * uwBatSerialPcs);
		bEEPROM2Flag = true;
	}

	if((suwGetEERlyVoltOff() > cBatVoltReal15V * uwBatSerialPcs) \
		|| (suwGetEERlyVoltOff() < cBatVoltReal11V * uwBatSerialPcs))
	{
		sSetEERlyVoltOff(strEepromDefaultTable2.strAcData.uwEERlyVoltOff * uwBatSerialPcs);
		bEEPROM2Flag = true;
	}

	if(suwGetEEGridCurrLimit() > uwGetGridInputCurrMax()) 
	{
		sSetEEGridCurrLimit(strEepromDefaultTable2.strAcData.uwEEGridCurrLimit);
		bEEPROM2Flag = true;
	}

	if(suwGetEEGridInputPower() > uwGetGridInputWattMax()) 
	{
...
{
    return uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd97;
}

INT16U	suwGetEERsvd98(void)
{
    return uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd98;
}

INT16U	suwGetEERsvd99(void)
{
    return uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd99;
}

INT16U	suwGetEERsvd100(void)
{
    return uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd100;
}

INT16U	suwGetEERsvd101(void)
{
    return uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd101;
}

INT16U	suwGetEERsvd102(void)
{
    return uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd102;
}

INT16U	suwGetEERsvd103(void)
{
    return uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd103;
}
...
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd97 = uwtemp;
    OS_EXIT_CRITICAL();
}
...
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd98 = uwtemp;
    OS_EXIT_CRITICAL();
}
...
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd99 = uwtemp;
    OS_EXIT_CRITICAL();
}
...
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd100 = uwtemp;
    OS_EXIT_CRITICAL();
}
...
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd101 = uwtemp;
    OS_EXIT_CRITICAL();
}

void	sSetEERsvd102(INT16U uwtemp)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd102 = uwtemp;
    OS_EXIT_CRITICAL();
}

void	sSetEERsvd103(INT16U uwtemp)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.strBatData.uwEERsvd103 = uwtemp;
    OS_EXIT_CRITICAL();
}
