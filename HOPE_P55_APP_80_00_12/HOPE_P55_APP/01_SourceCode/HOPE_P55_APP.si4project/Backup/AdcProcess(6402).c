...
#define		cADC_AVG_CNT		384			// 20003*52us=20ms

// P55版本的采样系数没有更新，需要重新计算
...
#define	cBatVoltRatio			990			// +3.96VMax	0.01V	K=100*3.3/(1.0*0.00008333)
#define	cBatVolt2Ratio			396			// +3.96VMax	0.01V	K=100*3.3/(1.0*0.085714)
#define	cBatCurrRatio			792//396 		// ±198.00AMax	0.01A	K=100*3.3/(50.0*0.085714)	1.65Vbias
#define	cPvVoltRatio			6600		// +66.00VMax	0.01V	K=100*3.3/(1.0*0.024510)
#define	cPvLCurrRatio			7920		// ±217.6AMax	0.01A	K=100*3.3/((49.9/15)*0.001)
...
#define	cB1VoltRatio			6600
#define	cB2VoltRatio			6600
#define	cBVoltRatio				396
#define	cPvInCurrRatio			660
#define	cPvInTzCurrRatio		660
...
//	sSetPvBatCurrBias(suwGetEEPV1BatCurrBias());
	sSetPvVoltAdj(4096);
	sSetPvVoltBias(0);
	sSetPvBatCurrAdj(4096);
	sSetPvBatCurrBias(0);
	sSetPvInCurrAdj(4096);
	sSetPvInCurrBias(0);
	sSetPvInTzCurrAdj(4096);
	sSetPvInTzCurrBias(0);
...
//		wAdcRealSumCntTemp[emuADC_RLINE_VOLT] = 0;
	...
	wAdcResult1[cADC_CH_INV_VOLT]			= ADCARESULT0 - 2048;		// INV.V
	wAdcResult1[cADC_CH_INV_CURR]			= ADCARESULT1 - 2048;		// INV.I	
	wAdcResult1[cADC_CH_OP_CURR]			= 2048 - ADCARESULT2;		// OP.I
	wAdcResult1[cADC_CH_VBAT_VOLT]			= ADCARESULT3;				// VBAT.V
	wAdcResult1[cADC_CH_BUS_VOLT]			= ADCARESULT4;				// BUS.V
	wAdcResult1[cADC_CH_GRID_VOLT]			= ADCARESULT5 - 2048;		// GRID.V
	wAdcResult1[cADC_CH_BAT_VOLT]			= ADCARESULT6;				// BAT.V
	wAdcResult1[cADC_CH_AC12V]				= ADCARESULT7;				// AC_12V.V
	wAdcResult1[cADC_CH_LLC_OCP]			= ADCARESULT8;				// LV_12V.V   第二版：LLC.OCP
//	wAdcResult1[cADC_CH_LV12V]				= ADCARESULT8;				// LV_12V.V   第二版：LLC.OCP
	wAdcResult1[cADC_CH_OP_VOLT]			= ADCARESULT9 - 2048;		// OP.V
	wAdcResult1[cADC_CH_LV12V]				= ADCARESULT10;				// LLC.OCP	   第二版：LV_12V.V
//	wAdcResult1[cADC_CH_LLC_OCP]			= ADCARESULT10 - 2048;		// LLC.OCP	   第二版：LV_12V.V
	wAdcResult1[cADC_CH_VAR]				= ADCARESULT11;				// VAR
	wAdcResult1[cADC_CH_BAT_CURR]			= ADCARESULT12 - 2048;		// BAT.I
	wAdcResult1[cADC_CH_B1_VOLT]			= ADCARESULT13;				// B1.V
	wAdcResult1[cADC_CH_INV_TEMP]			= ADCARESULT14;				// INV.T
	...
	while(AdccRegs.ADCSOCFLG1.all != 0);
...
void	sAdcDCDCIsr(void)
{
	...
	// Get InvIsr adc result
	while(AdcaRegs.ADCSOCFLG1.all != 0);
	wAdcResult2[cADC_CH_INV_VOLT]			= ADCARESULT0 - 2048;		// INV.V
	wAdcResult2[cADC_CH_INV_CURR]			= ADCARESULT1 - 2048;		// INV.I	
	wAdcResult2[cADC_CH_OP_CURR]			= 2048 - ADCARESULT2;		// OP.I
	wAdcResult2[cADC_CH_VBAT_VOLT]			= ADCARESULT3;				// VBAT.V
	wAdcResult2[cADC_CH_BUS_VOLT]			= ADCARESULT4;				// BUS.V
	wAdcResult2[cADC_CH_GRID_VOLT]			= ADCARESULT5 - 2048;		// GRID.V
	wAdcResult2[cADC_CH_BAT_VOLT]			= ADCARESULT6;				// BAT.V
	wAdcResult2[cADC_CH_AC12V]				= ADCARESULT7;				// AC_12V.V
	wAdcResult2[cADC_CH_LLC_OCP]			= ADCARESULT8;				// LV_12V.V   第二版：LLC.OCP
//	wAdcResult2[cADC_CH_LV12V]				= ADCARESULT8;			// LV_12V.V   第二版：LLC.OCP
	wAdcResult2[cADC_CH_OP_VOLT]			= ADCARESULT9 - 2048;		// OP.V
	wAdcResult2[cADC_CH_LV12V]				= ADCARESULT10; 			// LLC.OCP	   第二版：LV_12V.V
//	wAdcResult2[cADC_CH_LLC_OCP]			= ADCARESULT10 - 2048;	// LLC.OCP	   第二版：LV_12V.V
	wAdcResult2[cADC_CH_VAR]				= ADCARESULT11;				// VAR
	wAdcResult2[cADC_CH_BAT_CURR]			= ADCARESULT12 - 2048;		// BAT.I
	wAdcResult2[cADC_CH_B1_VOLT]			= ADCARESULT13;				// B1.V
	wAdcResult2[cADC_CH_INV_TEMP]			= ADCARESULT14;				// INV.T
...
//	wAdcReal[cADC_CH_BAT_VOLT]		= ((INT32S)wAdcReal[cADC_CH_B1_VOLT] + wAdcReal[cADC_CH_B2_VOLT])/10 ; 		// BAT.V
	...
	wAdcReal[cADC_CH_PVL_CURR]		= ((INT32S)wAdcReal[cADC_CH_PVIN_CURR] * (INT32S)wAdcReal[cADC_CH_PV_VOLT]) / (INT32S)wAdcReal[cADC_CH_BAT_VOLT]; 		// PV.I	
...
//		if(uwGetModelLV())
//		{
			if(g_wKpwm >= 1446) 	 // 3125*128/130V = 3077         1446
			{
				g_wKpwm = 1446;
			}
			else if(g_wKpwm <= 1085) // 3125*128/240V = 1667          1085
			{
				g_wKpwm = 1085;
			}
//		}
...
INT16S	swPv1BatWattCal(void)			// W	//未考虑倍率关系，应该还要除1000
{
...
********************************************************************************/
///////////////////////////////////////Result1
INT16S	swGetAdcInvVoltResult1(void)				// INV.V
{
	return wAdcResult1[cADC_CH_INV_VOLT];
}
INT16S	swGetAdcInvCurrResult1(void)				// INV.I
{
	return wAdcResult1[cADC_CH_INV_CURR];
}
INT16S	swGetAdcOpCurrResult1(void)					// OP.I
{
	return wAdcResult1[cADC_CH_OP_CURR];
}
INT16S	swGetAdcBatVolt2Result1(void)				// VBAT.V
{
	return wAdcResult1[cADC_CH_VBAT_VOLT];
}
INT16S	swGetAdcBusVoltResult1(void)				// BUS.V
{
	return wAdcResult1[cADC_CH_BUS_VOLT];
}
INT16S	swGetAdcGridVoltResult1(void)				// GRID.V
{
	return wAdcResult1[cADC_CH_GRID_VOLT];
}
INT16S	swGetAdcBatVoltResult1(void)				// BAT.V
{
	return wAdcResult1[cADC_CH_BAT_VOLT];
}
INT16S	swGetAdcAC12VoltResult1(void)				// AC_12V.V
{
	return wAdcResult1[cADC_CH_AC12V];
}
INT16S	swGetAdcLV12VoltResult1(void)				// LV_12V.V
{
	return wAdcResult1[cADC_CH_LV12V];
}
INT16S	swGetAdcOpVoltResult1(void)					// OP.V
{
	return wAdcResult1[cADC_CH_OP_VOLT];
}
INT16S	swGetAdcLLCOCPCurrResult1(void)				// LLC.OCP
{
	return wAdcResult1[cADC_CH_LLC_OCP];
}
INT16S	swGetAdcVarResult1(void)					// VAR
{
	return wAdcResult1[cADC_CH_VAR];
}
INT16S	swGetAdcBatCurrResult1(void)				// BAT.I
{
	return wAdcResult1[cADC_CH_BAT_CURR];
}
INT16S	swGetAdcB1VoltResult1(void)					// B1.V
{
	return wAdcResult1[cADC_CH_B1_VOLT];
}
INT16S	swGetAdcPv1BatCurrResult1(void)				// PV.BAT.I	// PV电感电流
{
	return wAdcResult1[cADC_CH_PVL_CURR];
}
INT16S	swGetAdcPvInTzCurrResult1(void)				// PVIN.I.TZ
{
	return wAdcResult1[cADC_CH_PVINTZ_CURR];
}
INT16S	swGetAdcPvInCurrResult1(void)				// PVIN.I
{
	return wAdcResult1[cADC_CH_PVIN_CURR];
}
INT16S	swGetAdcPv1VoltResult1(void)				// PV.V
{
	return wAdcResult1[cADC_CH_PV_VOLT];
}
INT16S	swGetAdcB2VoltResult1(void)					// B2.V
{
	return wAdcResult1[cADC_CH_B2_VOLT];
}
INT16S	swGetAdcBVoltResult1(void)					// B.V
{
	return wAdcResult1[cADC_CH_B_VOLT];
}


///////////////////////////////////////Result2
INT16S	swGetAdcInvVoltResult2(void)				// INV.V
{
	return wAdcResult2[cADC_CH_INV_VOLT];
}
INT16S	swGetAdcInvCurrResult2(void)				// INV.I
{
	return wAdcResult2[cADC_CH_INV_CURR];
}
INT16S	swGetAdcOpCurrResult2(void)					// OP.I
{
	return wAdcResult2[cADC_CH_OP_CURR];
}
INT16S	swGetAdcBatVolt2Result2(void)				// VBAT.V
{
	return wAdcResult2[cADC_CH_VBAT_VOLT];
}
INT16S	swGetAdcBusVoltResult2(void)				// BUS.V
{
	return wAdcResult2[cADC_CH_BUS_VOLT];
}
INT16S	swGetAdcGridVoltResult2(void)				// GRID.V
{
	return wAdcResult2[cADC_CH_GRID_VOLT];
}
INT16S	swGetAdcBatVoltResult2(void)				// BAT.V
{
	return wAdcResult2[cADC_CH_BAT_VOLT];
}
INT16S	swGetAdcAC12VoltResult2(void)				// AC_12V.V
{
	return wAdcResult2[cADC_CH_AC12V];
}
INT16S	swGetAdcLV12VoltResult2(void)				// LV_12V.V
{
	return wAdcResult2[cADC_CH_LV12V];
}
INT16S	swGetAdcOpVoltResult2(void)					// OP.V
{
	return wAdcResult2[cADC_CH_OP_VOLT];
}
INT16S	swGetAdcLLCOCPCurrResult2(void)				// LLC.OCP
{
	return wAdcResult2[cADC_CH_LLC_OCP];
}
INT16S	swGetAdcVarResult2(void)					// VAR
{
	return wAdcResult2[cADC_CH_VAR];
}
INT16S	swGetAdcBatCurrResult2(void)				// BAT.I
{
	return wAdcResult2[cADC_CH_BAT_CURR];
}
INT16S	swGetAdcB1VoltResult2(void)					// B1.V
{
	return wAdcResult2[cADC_CH_B1_VOLT];
}
INT16S	swGetAdcPv1BatCurrResult2(void)				// PV.BAT.I	// PV电感电流
{
	return wAdcResult2[cADC_CH_PVL_CURR];
}
INT16S	swGetAdcPvInTzCurrResult2(void)				// PVIN.I.TZ
{
	return wAdcResult2[cADC_CH_PVINTZ_CURR];
}
INT16S	swGetAdcPvInCurrResult2(void)				// PVIN.I
{
	return wAdcResult2[cADC_CH_PVIN_CURR];
}
INT16S	swGetAdcPv1VoltResult2(void)				// PV.V
{
	return wAdcResult2[cADC_CH_PV_VOLT];
}
INT16S	swGetAdcB2VoltResult2(void)					// B2.V
{
	return wAdcResult2[cADC_CH_B2_VOLT];
}
INT16S	swGetAdcBVoltResult2(void)					// B.V
{
	return wAdcResult2[cADC_CH_B_VOLT];
}

///////////////////////////////////////Sample
INT16S	swGetAdcInvVoltSample(void)					// INV.V
{
	return wAdcSample[cADC_CH_INV_VOLT];
}
INT16S	swGetAdcInvCurrSample(void)					// INV.I
{
	return wAdcSample[cADC_CH_INV_CURR];
}
INT16S	swGetAdcOpCurrSample(void)					// OP.I
{
	return wAdcSample[cADC_CH_OP_CURR];
}
INT16S	swGetAdcBatVolt2Sample(void)				// VBAT.V
{
	return wAdcSample[cADC_CH_VBAT_VOLT];
}
INT16S	swGetAdcBusVoltSample(void)					// BUS.V
{
	return wAdcSample[cADC_CH_BUS_VOLT];
}
INT16S	swGetAdcGridVoltSample(void)				// GRID.V
{
	return wAdcSample[cADC_CH_GRID_VOLT];
}
INT16S	swGetAdcBatVoltSample(void)					// BAT.V
{
	return wAdcSample[cADC_CH_BAT_VOLT];
}
INT16S	swGetAdcAC12VoltSample(void)				// AC_12V.V
{
	return wAdcSample[cADC_CH_AC12V];
}
INT16S	swGetAdcLV12VoltSample(void)				// LV_12V.V
{
	return wAdcSample[cADC_CH_LV12V];
}
INT16S	swGetAdcOpVoltSample(void)					// OP.V
{
	return wAdcSample[cADC_CH_OP_VOLT];
}
INT16S	swGetAdcLLCOCPCurrSample(void)				// LLC.OCP
{
	return wAdcSample[cADC_CH_LLC_OCP];
}
INT16S	swGetAdcVarSample(void)						// VAR
{
	return wAdcSample[cADC_CH_VAR];
}
INT16S	swGetAdcBatCurrSample(void)					// BAT.I
{
	return wAdcSample[cADC_CH_BAT_CURR];
}
INT16S	swGetAdcB1VoltSample(void)					// B1.V
{
	return wAdcSample[cADC_CH_B1_VOLT];
}
INT16S	swGetAdcPv1BatCurrSample(void)				// PV.BAT.I	// PV电感电流
{
	return wAdcSample[cADC_CH_PVL_CURR];
}
INT16S	swGetAdcPvInTzCurrSample(void)				// PVIN.I.TZ
{
	return wAdcSample[cADC_CH_PVINTZ_CURR];
}
INT16S	swGetAdcPvInCurrSample(void)				// PVIN.I
{
	return wAdcSample[cADC_CH_PVIN_CURR];
}
INT16S	swGetAdcPv1VoltSample(void)					// PV.V
{
	return wAdcSample[cADC_CH_PV_VOLT];
}
INT16S	swGetAdcB2VoltSample(void)					// B2.V
{
	return wAdcSample[cADC_CH_B2_VOLT];
}
INT16S	swGetAdcBVoltSample(void)					// B.V
{
	return wAdcSample[cADC_CH_B_VOLT];
}
...
INT16S	swGetAdcVarReal(void)					// VAR
{
	return wAdcReal[cADC_CH_VAR];
}
INT16S	swGetAdcBatCurrReal(void)				// BAT.I
{
	return wAdcReal[cADC_CH_BAT_CURR];
}
INT16S	swGetAdcB1VoltReal(void)				// B1.V
{
	return wAdcReal[cADC_CH_B1_VOLT];
}
INT16S	swGetAdcPv1BatCurrReal(void)			// PV.BAT.I	// PV电感电流
{
	return wAdcReal[cADC_CH_PVL_CURR];
}
INT16S	swGetAdcPvInTzCurrReal(void)			// PVIN.I.TZ
{
	return wAdcReal[cADC_CH_PVINTZ_CURR];
}
INT16S	swGetAdcPvInCurrReal(void)				// PVIN.I
{
	return wAdcReal[cADC_CH_PVIN_CURR];
}
INT16S	swGetAdcPv1VoltReal(void)				// PV.V
{
	return wAdcReal[cADC_CH_PV_VOLT];
}
INT16S	swGetAdcB2VoltReal(void)				// B2.V
{
	return wAdcReal[cADC_CH_B2_VOLT];
}
INT16S	swGetAdcBVoltReal(void)					// B.V
{
	return wAdcReal[cADC_CH_B_VOLT];
}
