...
void	sPvChgModeProc(void)
{
	...
	{
		uwPvChgBatCurrTemp = g_wBatChgCurrLimit;	// 自动获取EEPROM设置/用户设置
	}
	else
	{
...
//	Uplimit(uwPvChgBatCurrTemp,swGetTempDeratPvBatCurrFilter());	// PV给电池的充电电流给定，不能超过额定电流
	...
	Uplimit(uwPvChgCurrTemp,200);	// PV侧输入限制20A
	...
	{
		if(uwPvChgBatCurrRef < uwPvChgCurrTemp)	// PV控制器的目标电流 0.01A
		{
			uwPvChgBatCurrRef += 1;//2; //5;	// 1
		}
		else
		{
...
//	else
//	{
//		sSetPvChgBatVoltRef(cBatVoltReal3V25*uwGetBatSerialPcs());
//	}
