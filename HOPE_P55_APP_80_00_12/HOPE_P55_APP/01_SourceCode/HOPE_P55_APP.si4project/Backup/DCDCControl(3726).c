...
INT16U gi_wChgModeFlagEn = 0;							// 低压侧占空比
...
INT16U gi_wDCDCLowPWMDutyLimit = 100;					//低压侧占空比限制
...
INT16U g_uwLowPauseTime = 1000;	
INT16U g_uwLowSoftDutyRate = 1000;
...
void	sOpenLLCCtrl(void)
{
	...
	if(!mChkDCDCLLCOn())
	{
		...
		gi_wDCDCLowPWMDutyLimit = 100;					//低压侧占空比限制
	...
	else
	{
		...
		if(gi_wChgModeFlagEn == true)//(gi_wGridChgBatLowCutFlag == true))//&&(suwGetFBInvCtrlSts() == cFBInvCtrlBus))
		{
		  //低压侧封波              高压侧软起
			...
			if(swGetTriChgFlag() == false)            //退出涓流时增加高压侧占空比直至打满, 且低压侧打占空比
			{
				...
				if(g_uwLowPauseTime > 0)		// 进入发波，开启高压侧先进行一段时间封波保持，不立即发波
				{
					g_uwLowPauseTime--;
//					mDisDCDCForcePWMOut();
//					mDisDCDCHForcePWMOut();
				...
				else if(gi_wDCDCLowPWMDuty < gi_wDCDCLowPWMDutyLimit)					//高压侧占空比限制)		// 占空比软起
				{
...
//					mDisDCDCForcePWMOut();
			...
			else
			{
				...
				mEnDCDCHPWMOut();
				mEnDCDCHForcePWMOut();	
				if(gi_wDCDCLowPWMDutyLimit > 0)
				{
					gi_wDCDCLowPWMDutyLimit--;
				}
				if(gi_wDCDCHighPWMDutyLimit > gi_wDCDCHighPWMDutyLimitInit)
				{
					gi_wDCDCHighPWMDutyLimit = gi_wDCDCHighPWMDutyLimitInit;
				}
			}
			
			//高压侧占空比大于低压侧占空比
			if(gi_wDCDCLowPWMDutyLimit < gi_wDCDCHighPWMDutyLimit - 20)
			{
				gi_wDCDCLowPWMDutyLimit =  gi_wDCDCHighPWMDutyLimit - 20;
			}
...
//		if((swGetAdcBusVoltReal() >= 3000) && ((gi_wDCDCConvPWMValue > 300) || (gi_wDCDCConvPWMValue1 > 100))
...
//
//		mDCDCConvPRD34 = gi_wDCDCConvPWMPeriod; // H
//		mDCDCConvPWM3 = gi_wDCDCConvPWMPeriod - gi_wDCDCConvPWMValue1;//cDCDCConvertDuty1;
//		mDCDCConvPWM4 = gi_wDCDCConvPWMValue1;//cDCDCConvertDuty1;
