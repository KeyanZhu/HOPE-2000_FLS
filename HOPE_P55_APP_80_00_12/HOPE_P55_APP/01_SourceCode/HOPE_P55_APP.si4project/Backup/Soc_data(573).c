...
{
	sSocInput.wMaxCellVoltage = swGetMaxCellVoltNew(); 	     //1mV
	sSocInput.wMinCellVoltage = swGetMinCellVoltNew();  	 //1mV
	sSocInput.wMaxCellTemplate = swGetMaxCellTempNew();      //1℃
	sSocInput.wMinCellTemplate = swGetMinCellTempNew();      //1℃
	sSocInput.wCurrent = swGetBattCurrNew(); 		   		 //0.1A
	sSocInput.wBattVoltage = swGetBattVoltNew();			 //0.01V
	
	sSocInput.bNowSoc = swGetSocNew();                       //1%
	sSocInput.wRealCapacity = swGetSoxAhRealShow();		     //0.1Ah
	
	if(sSocInput.bCellNum != swGetBmsCellVoltNum())
	{
		sSocInput.bCellNum = swGetBmsCellVoltNum();	   		 //电芯串数
	}
	if(sSocInput.wRatedCapacity != swGetBmsRatedAH())
	{
		sSocInput.wRatedCapacity = swGetBmsRatedAH();	   	 //0.1Ah
	}
...
void sSoc_Updata(void)
{
	...
	if(wBmsStatusLow & 0x2000) //充电
	{
		...
		if(sSocData.bBattMode != BATT_MODE_CHG)
		{
			...
			for (int i = 0; i < CHG_CURRENT_LIST_MAXNUM; i++) 
			{
	...
	else
	{
		...
		if(sSocData.bBattMode != BATT_MODE_DHG)
		{
			...
			for (int i = 0; i < DHG_CURRENT_LIST_MAXNUM; i++) 
			{
				sSocData.bCurrentList[i] = 0;
	...
	if((bBmsMode == cPowerOnMode) || (bBmsMode == cStandbyMode))
	{
