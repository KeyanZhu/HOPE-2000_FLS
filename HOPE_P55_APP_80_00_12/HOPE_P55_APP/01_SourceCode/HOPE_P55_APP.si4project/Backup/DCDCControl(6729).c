...
/*
		if((gi_wGridChgBatLowCutFlag == true))//&&(suwGetFBInvCtrlSts() == cFBInvCtrlBus))
		{
			if(g_uwLLCPauseTime > 0)		// 进入发波，先进行一段时间封波保持，不立即发波
			{
				...
				mDisDCDCForcePWMOut();
				mDisDCDCHForcePWMOut();
				gi_wDCDCConvPWMValue1 = 0;
				gi_wDCDCConvPWMPeriod = uwGetDcDcPWMPeriod();
			}
			else if(gi_wDCDCConvPWMValue1 < 120)		// 占空比软起
			{
				if(++uwLLCSoftCnt >= g_uwLLCSoftDutyRate)
				{
					uwLLCSoftCnt = 0;
					gi_wDCDCConvPWMValue1++;
				}
