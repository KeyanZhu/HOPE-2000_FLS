	...
	while(1)
	{
		...
		if(wEvent & (1 << eSuperTimer))
		{
			...
			{	
				...
				{
					if(suwGetChgModeFlagEn() == true)   //chg-->bat -->chg
					{
						//cChgMode模式
						if(uwChgOKDelay < 500)	// 10S
						{
							uwChgOKDelay++;
						}
						else if(1) 
						{
							uwChgOKDelay = 0;
							OSEventSend(cPrioSuper, eSuperToChg);	// for test
						}
				...
				else		//高压并网
				{
					if(suwGetChgModeFlagEn() == true)			//chg-->bat -->chg
					{
						...
						if((swGetTriChgFlag() == true))  //涓流模式下  回到待机 再启机
						{
...
//					uwLineOKDelay++;
//				}
////				if(uwLineOKDelay < (suwGetEEFeedToLineTime() / cTimerBase))	// 可设置
////				{
////					uwLineOKDelay++;
////				}
//				else if(1) 
