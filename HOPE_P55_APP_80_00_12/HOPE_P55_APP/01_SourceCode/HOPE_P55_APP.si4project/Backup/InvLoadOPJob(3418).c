...
//
			...
			sOPCurrOverChk(500,10);										// OP过流    25A 
...
//	INT16S wInvChgCurrMaxTemp;
	...
	if(suwGetPV1CtrlSts() == cPVWork)	// PV控制器工作时，才计算相关参数
	{
		...
		Uplimit(wPvChgPowerTemp,600);															// 额定1600W
	}
	else
	{
		wPvChgCurrTemp  = 0;
...
//	wRlyChgPowerLimitTemp = ((INT32S)wInvChgCurrMaxTemp * swGetRLineVoltNew()) / 1000;
	...
	UpDownLimit(wRlyChgPowerLimitTemp,500,-500);	// ±500W
...
//		wInvCurrTemp = (INT16S)((INT32S)wInvCtrlPower * 1414 / swGetRLineVoltNew());	// 主计算算出的并网充/带载电流
//		UpDownLimit(wInvCurrTemp,gi_wInvChgCurrLimitTemp,-gi_wInvFeedCurrLimitTempTemp);
//		wInvCurrFeedTemp = (INT16S)((INT32S)g_wACFeedPowerLimit * 1414 / swGetRLineVoltNew()); // 设置的卖电功率
//		UpDownLimit(wInvCurrFeedTemp,gi_wInvChgCurrLimitTemp,-gi_wInvFeedCurrLimitTempTemp);
...
//	}
	...
	if((wWattTemp > uwGetInvWattMax()) && (swGetWorkMode() == cBatteryMode))//uwGetInvWattMax()
	{
		uwWattDeratePerTemp = (wWattTemp - uwGetInvWattMax()) / 10;//uwGetInvWattMax()
		wWattDeratROpVolt -= (uwWattDeratePerTemp);
		if(wWattDeratROpVolt < uwGetInvDeratingVolt())
		{
			wWattDeratROpVolt = uwGetInvDeratingVolt();
		}
//		sSetEEACOutputRatedVolt(wWattDeratROpVolt);
	}
	else
	{
