...
//			sSetHsTwTempNew(swHsTwTempCal());		// TW.T
			...
			sSetMinCellTempNew();					// 电芯温度参数更新
			sSetMinCellVoltNew();					// 电芯电压参数更新
			sBatteryParaUpdate();					// 电池参数更新

			sBatVoltRefAdj();						// 
			sBatPercentCal();						// SOC预估(电压) 量产不要这个
			...
			sPv1UnderChk(cT5sec,cT200ms);												// PV欠压告警,11.5V,5S

			// BAT
			sBatVoltOverChk((cBatVoltReal3V65*uwGetBatSerialPcs()), cT5sec);	// 电池过压故障,3.65V,5S
			
			// 函数内部检测阈值未适配HOPE
			sBatOVAlarmChk((cBatVoltReal3V6*uwGetBatSerialPcs()), cT200ms);
			sBatUnderChk(cT200ms);												// 电池欠压告警,200ms
			sBatCurrOverChk(260, cT5sec,330,cT500ms);										// 200A
			
			sBatOpenChk((cBatVoltReal2V08*uwGetBatSerialPcs()), cT1sec);		// 电池开路检测,单节2.12V,200ms

			// 函数内部检测阈值未适配HOPE
			sLowBatCutOffChk(cT3sec);											// 电池低压关输出检测					// 待优化

			// BUS
			sBusOverChk(4950, cT1sec, 5000, cT100ms);							// BUS过压检测故障,1段,1s   ,1段,100ms
