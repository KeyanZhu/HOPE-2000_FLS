...
extern INT16U g_uwBatSoftOCurrCnt;           // 电池软件过压故障
...
INT16U g_uwStayStandby = true;	// 默认待机，不允许自动启动主功率
...
void	sPowerOnMode(void)
{
	...
	sSetBATSPSRlyOn(true);	// 这个信号没有用
	
	while(1)
	{
		bNextMode = sbWaitSystemSteady(cT100ms);
		/*
...
//		}
		...
		bNextMode = sbAdcOffsetInitial(cT100ms);
		/*
		if(bNextMode == cShutdownMode)
		{
			g_uwWorkMode = cShutdownMode;
...
//	mDisPV1PWMOut();			// PV封波
	...
	if(uwFaultCodeTemp == cBatCurrOver)		// 电池软件过压故障
	{
		g_uwBatSoftOCurrCnt++;
	}
	...
	while(1)
	{
		...
		if(wEvent & (1 << eSuperTimer))
		{
			...
			if(uwFaultCodeTemp == cBatCurrOver)	
			{
				if(T_FAULT_DATA.bits.bBatSoftOC == false && g_uwBatSoftOCurrCnt < 3)
				{
					uwFaultRestartFlg = true;
				}
				else
...
void	sStandbyMode(void)
{
	...
	sSetFBInvCtrlSts(cFBWait);	// 逆变待机
	sSetPV1CtrlSts(cPVWait);	// PV待机
	sSetChgModeFlagEn(false);	//默认不进入chgmode
	
	g_uwLoadOnSts = 0;			// 负载模式待机
//	g_wToChgModeFlag == false;
	sSetRSinAmp(0);
	sSetRNewSinAmp(0);

	hoSTAROn;		// 电池软起/预充，到这里了默认认为电容已经充电完成，关掉预充，下一步打开充放电MOS管
	hoSRDRVOn;		// 电池充放电MOS使能
	mSPSENOn;		// 12V驱动辅源使能
...
//					else 
					...
					else if(g_uwLoadOnSts == cToChgMode)
					{
						g_uwWorkMode = cToChgMode;
						return;
					}
					else if(g_uwLoadOnSts == cFBWait)
					{
						g_uwWorkMode = cStandbyMode;
						return;
					}
			...
			&& (g_uwWorkMode != cFaultMode) && (g_uwFaultCode == 0)\
			&& (T_ALARM_DATA.bits.bBatOpenAlarm == false) && (T_ALARM_DATA.bits.bAlarmLowBatCutOff == false) && (sGetGridChgBusVoltFlag() == false))
			{
				sSetChgModeFlagEn(false);
				if((s_OnInvDelay++) >= cT1sec)	// 1S
				{
					s_OnInvDelay = 0;
				...
				&& (T_ALARM_DATA.bits.bGridOF == false))
			{
				if((T_ALARM_DATA.bits.bAlarmLowBatCutOff == true) || (sGetGridChgBusVoltFlag() == true))
				{
					sSetChgModeFlagEn(true);
					if((s_OnChgDelay++) >= cT1sec)	
					{
						s_OnChgDelay = 0;
						/*******************************************************************	
...
//						{
//							g_wToChgModeFlag = true;
//							sSetChgModeFlagEn(true);
							sSetLoadOnSts(cToChgMode);
//						}
					}
				}
				else
...
//				mLED1Off;
				...
				sSetInvRMSCtrlVolt(1840);	// 初始值
				sSetRSinAmp(8326);
				sSetRNewSinAmp(8326);
//				sSetRSinAmp(10408);
//				sSetRNewSinAmp(10408);
				sSetRSinPointer(0);
				sSetRSinPionterQ15(0);
				sSetLoadOnSts(2);
        ...
        if(wEvent & (1 << eSuperTimer))
        {
			...
			|| (T_ALARM_DATA.bits.bGridAbnormalAlarm == true))//|| (sGetGridChgBusVoltFlag() == true))
			{	
			...
			else if(suwGetFBInvCtrlSts() != cFBInvCtrlBus)	// 可以考虑优化成在过零点切入并网控制器？
			{
...
//	static INT16U uwChgOKDelay = 0;
	static INT16U uwStandyOKDelay = 0;
//	static INT16U uwOffInvDelay = 0;
	...
	while(1)
	{
		...
		if(wEvent & (1 << eSuperToLine))
		{
			...
			sSetFBInvCtrlSts(cFBWait);
			sRlyDelayProc(RLY_DELAY,cDelay9ms,eSuperSelectPoint,cPrioSuper);			// 延时7ms延时打开继电器，当继电器吸合时刚好在过零点
			sOSTimerStart();
		...
		if(wEvent & (1 << eSuperToChg))
		{
			sSetOPRelayOn(false);
			sSetLoadOnSts(cToChgMode);
			sOSTimerStop();
			sRlyDelayProc(RLY_CROSSZERO,cGRIDRlyDelayTime,eSuperSelectPoint,cPrioSuper);// 检测市电过零点
			sRlyDelayProc(RLY_DELAY,cDelay7ms,eSuperSelectPoint,cPrioSuper);			// 延时7ms延时打开继电器，当继电器吸合时刚好在过零点
			sOSTimerStart();
			sSetFBInvCtrlSts(cFBWait);
			sSetGridRelayOn(true);		// 吸合市电继电器
		...
		if(wEvent & (1 << eSuperTimer))
		{
			...
			&& (T_ALARM_DATA.bits.bGridUV == false) && (T_ALARM_DATA.bits.bGridOV == false)\
			&& (T_ALARM_DATA.bits.bGridUF == false) && (T_ALARM_DATA.bits.bGridOF == false))
			{	
				if(sGetGridChgBusVoltFlag() == false)
				{
						//LineMode模式
						if(uwLineOKDelay < 500)	// 10S  
						{
							uwLineOKDelay++;
						}
						else if(1) 
						{
							uwLineOKDelay = 0;
							OSEventSend(cPrioSuper, eSuperToLine);	// for test
						}
				}
				else		//高压并网
				{
						//进入待机模式
						if(uwStandyOKDelay < 500)	// 10S
						{
							uwStandyOKDelay++;
						}
						else if(1) 
						{
							uwStandyOKDelay = 0;
							OSEventSend(cPrioSuper, eSuperToStandby);	// for test
						}
				}
			...
			else	// 离网模式运行
			{
				uwLineOKDelay = 0;
				//hoGRIDRLYOff;						
				sSetGridRelayOn(false);				// 保证市电继电器断开
...
//					sOSTimerStart();
				...
				{
//					sOSTimerStop();
					sRlyDelayProc(RLY_DELAY,cDelay9ms,eSuperSelectPoint,cPrioSuper);
//					sOSTimerStart();
				}
				sSetFBInvCtrlSts(cFBInvCtrlVolt);
				sSetLoadOnSts(2);
	...
	/*	
		...
		*/
		if(wEvent & (1 << eSuperTimer))
		{
			if((T_ALARM_DATA.bits.bGridUV == true) || (T_ALARM_DATA.bits.bGridOV == true)\
			|| (T_ALARM_DATA.bits.bGridUF == true) || (T_ALARM_DATA.bits.bGridOF == true))//\
//			|| (sGetGridChgBusVoltFlag() == false))// ||((T_ALARM_DATA.bits.bAlarmLowBatCutOff == false)) )
			{	
				OSEventSend(cPrioSuper, eSuperToBat);
			}
			...
			if((g_uwLoadOnSts != 1) && (g_uwLoadOnSts != 2) && (g_uwLoadOnSts != 16))
//			if((g_uwLoadOnSts != 16))
			{
				sSetOPRelayOn(false);
				sSetGridRelayOn(false);
    ...
    while(1)
    {
        ...
        if(wEvent & (1 << eSuperTimer))
        {
			...
			else if(wToLineMode == 1)		// 软起BUS
			{
				...
				{
//					if((T_ALARM_DATA.bits.bAlarmLowBatCutOff == true))
//					{
//						sSetGridChgBatLowCutFlag(true)   //电池低压充电
//					}
//					else
//					{
//						sSetGridChgBatLowCutFlag(false)   //正常推挽软起
//					}
...
void	sToBatteryMode(void)
{
	INT16U wEvent;
	INT16U wToBatMode = 0;
	INT16U wBusSoftTimeout = 3000;
	INT16S wInvSoftTimeOutDelay = 500;
		...
		if(wEvent & (1 << eSuperTimer))
		{
			...
			else
			{
				...
				else if(wToBatMode == 2)	// 软起逆变
				{
					...
					{
						sSetInvRMSCtrlVolt(uwGetInvRMSCtrlVolt() + 100);	// 软起步进1V,20ms调整一次，相当于一秒50V
					}
					else
					{
						sSetInvRMSCtrlVolt(uwGetOPRMSRatedVolt());
					...
					if(--wInvSoftTimeOutDelay == 0)	// 500 大约是10S
					{
						wInvSoftTimeOutDelay = 500;
...
//	sSetGridRelayOn(false);				
    ...
    while(1)
    {
        ...
        if(wEvent & (1 << eSuperTimer))
        {
//            if((swGetBusVoltNew() > 3000))// && (g_wToChgModeFlag == true))
			if(g_uwLoadOnSts == cToChgMode)
        	{
				if(wToChgMode == 0)
				{
					wToChgMode = 1;
				}
				else if(wToChgMode == 1)
				{
					sSetOPRelayOn(true);		// 吸合负载继电器	
//					sRlyDelayProc(RLY_CROSSZERO,cGRIDRlyDelayTime,eSuperSelectPoint,cPrioSuper);// 检测市电过零点
//					sRlyDelayProc(RLY_DELAY,cDelay5ms,eSuperSelectPoint,cPrioSuper);			// 延时7ms延时打开继电器，当继电器吸合时刚好在过零点
//					sSetGridRelayOn(true);		// 吸合市电继电器
					if(++wDelayCnt > cT2sec)
					{
						wDelayCnt = 0;
				...
				else if(wToChgMode == 2)
				{
//					sSetOPRelayOn(true);		// 吸合负载继电器	
					sRlyDelayProc(RLY_CROSSZERO,cGRIDRlyDelayTime,eSuperSelectPoint,cPrioSuper);// 检测市电过零点
					sRlyDelayProc(RLY_DELAY,cDelay7ms,eSuperSelectPoint,cPrioSuper);			// 延时7ms延时打开继电器，当继电器吸合时刚好在过零点
					sSetGridRelayOn(true);		// 吸合市电继电器
					if((++wDelayCnt1 > cT1sec))//&& (uwGetGridRelayOn() == true) && (suwGetFBInvCtrlSts() == cFBInvCtrlBus))
					{
						wDelayCnt1 = 0;
						 wToChgMode = 3;
					}
				}
				else if(wToChgMode == 3)
				{
					if(++wDelay1 > cT3sec)
					{
						wDelay1 = 0;
					...
					if((swGetBusVoltNew() > 3850) && (++wDelayCnt2 > cT300ms))//待测试
					{
						wDelayCnt2 = 0;
						 wToChgMode = 4;
					}
				}
				else if(wToChgMode == 4)
				{
					if(suwGetDCDCCtrlSts() == cDCDCWork)// && suwGetDCDCSoftDutyFinish())
					{
						wBusSoftTimeout = 3000;
