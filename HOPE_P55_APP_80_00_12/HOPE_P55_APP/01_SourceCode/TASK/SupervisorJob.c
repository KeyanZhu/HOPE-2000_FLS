...
//	static INT16U uwOffInvDelay = 0;
	...
	while(1)
	{
		...
		if(wEvent & (1 << eSuperToLine))
		{
			...
			sSetFBInvCtrlSts(cFBWait);
			sRlyDelayProc(RLY_DELAY,cDelay8ms,eSuperSelectPoint,cPrioSuper);			// 延时7ms延时打开继电器，当继电器吸合时刚好在过零点
			sOSTimerStart();
//			sSetFBInvCtrlSts(cFBWait);
...
//					sSetOPRelayOn(true);		// 吸合负载继电器	
				...
				else if(wToChgMode == 3)
				{
					...
					if((++wDelayCnt2 > cT3sec))//&& (uwGetGridRelayOn() == true) && (suwGetFBInvCtrlSts() == cFBInvCtrlBus))
					{
						wDelayCnt2 = 0;
						 wToChgMode = 4;
					}
