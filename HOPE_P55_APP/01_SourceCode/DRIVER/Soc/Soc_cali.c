...
static INT8U sSocV_CaliCheck(void)
{
	...
	if(sSocData.bEmptyFlag) //放空
	{
	    ...
	    sSocData.wVoltCaliRealCapacity = (INT16U)((INT32U)sSocData.bVoltageCaliSoc * sSocData.wVoltCaliMaxCapacity / 100);
...
void sSoc_MaxAh_Cali(void)
{
	...
	if(sSocData.bCapacityCaliFlag == CAPACITY_CALI_ENABLE)
	{
		...
		if(T_ALARM_DATA.bits.bBatUVAlarm) //低压告警 HOPE 20260312
		{
			...
			sSocData.wCapaCaliMaxCapacity = (INT16U)((INT32U)sSocData.wMaxCapacity * bMaxAhFactor);
			sSocData.wCapaCaliRealCapacity = (INT16U)((INT32U)sSocData.bCapacityCaliSoc * sSocData.wCapaCaliMaxCapacity / 100);
			return;
		}
		...
		for(i = 0; i < (sizeof(bLowTempValue) / sizeof(bLowTempValue[0])); i++)
		{
