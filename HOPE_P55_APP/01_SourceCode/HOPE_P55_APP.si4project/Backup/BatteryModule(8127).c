...
INT16S g_wTriChgCurrLimitMax = 25;
INT16S g_wTriChgPowerLimit = 60;
INT16S g_wTriChgCurrLimit = 25;
...
INT16S wPvInCurrNew;				// 0.01A
INT16S wPvInTzCurrNew;				// 0.01A
...
INT16S g_wPv1UnderVolt = cMpptPvVolt12V;			// PV欠压告警值
INT16S g_wPv1UnderBackVolt = cMpptPvVolt12V + 100;	// PV欠压告警恢复值
...
void	sLowBatCutOffChk(INT16U uwFilter)
{
	...
	if(1)
	{
		...
		g_wLowBatCutOffVoltBackVolt = cBatVoltReal2V63 * uwBatSerialPcs;
	}
	else
	{
...
INT16S	swPvWattCal(void)
{
	...
	wBatWattTemp = (INT16S)(((INT32S)wPv1VoltNew * wPv1CurrNew + 5000) / 10000);
...
void	sBatCurrOverChk(INT16U uwBatCurrOverLevel, INT16U uwFilter)
{
	static INT16U s_uwBatCurrChkCnt = 0;
	
	if(T_FAULT_DATA.bits.bDcDcSoftOC == true)
	{
		if(sb16UUnderLevelChk(wBatCurrNew, uwBatCurrOverLevel - 500, uwFilter, &s_uwBatCurrChkCnt))
		{
			T_FAULT_DATA.bits.bDcDcSoftOC = false;
		}
	}
