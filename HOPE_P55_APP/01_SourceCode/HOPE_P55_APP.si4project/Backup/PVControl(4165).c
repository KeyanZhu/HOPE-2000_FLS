...
//		if(g_wBatVoltErr1 <= 0) // Bat voltage loop PI controler (电池满，调稳电池电压)
	...
	// PV充电状态识别
	//	if(swGetPv1BatCurrNew() >= cMpptBatCurr20A)	// 考虑增加计数滤波
...
//		mPV1BuckPWMA = cBuckPeriod - g_wPv1PWM;	// 占空比由调试寄存器写入
//		mPV1BuckPWMB = cBuckPeriod - g_wPv1PWM;	// 占空比由调试寄存器写入
	}
}
